...
class ProtLigTrainer(BaseTrain):
    ...
    def train(self):
        ...
        self.train = graph.get_tensor_by_name('training/train:0')
    ...
    def train_epoch(self):
        ...
        for bi, bj in batches('training'):
            ...
            self.session.run(self.train, feed_dict = {x: self.data.g_batch('training', x_t[bi:bj]), t: y_t[bi:bj]})
            train_err = self.session.run(self.mse, feed_dict = {x: self.data.g_batch('training', x_t[bi:bj]), t: y_t[bi:bj]})
